...
select * from products where products.user_id = 2;

DELETE FROM products WHERE id > 0;

insert into cupons(user_id, codigo, desconto) values ("user_id", "code", "desconto");

SELECT cupons.desconto FROM cupons, users WHERE cupons.codigo = "BRA10" and users.id = 2;
SELECT products.id from products where products.codigo = "";

insert into vendas(user_id, formpag_id, cupom_id, total) values
(2, 3, 1, 123123);

select * from vendas;
SELECT id FROM formpag WHERE id = 2;
select * from cupons;
delete from cupons where id = 3;